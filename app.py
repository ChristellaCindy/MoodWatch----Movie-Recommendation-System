...
st.title("🎬 Semantic Movie Recommender (A/B Testing)")
...
with st.expander("🤔 What is this & How does it work?", expanded=True):
    ...
    **How to Use:**
    1. Provide LLM API Key in the left sidebar.
    2. Describe your current mood or the type of movie you want to watch in the text area below.
    3. Click the button to get 2 different mysterious recommendation packages (Pack X and Pack Y).
    4. Read the synopses and fill out the evaluation form below to support our research!
    """)
...
if st.button("🚀 Get Movie Recommendations!", use_container_width=True):
    if not kunci_api:
        st.error("🚨 Please enter your LLM API Key in the left sidebar...")
    elif not curhatan_user:
        st.warning("⚠️ The movie description cannot be empty!")
    else:
        with st.spinner("🤖 Processing your request..."):
            try:
                # Ekstrak LLM
                keywords_ekstrak = extract_keywords_llm(kunci_api, curhatan_user)
...
if st.session_state['eval_ready']:
    ...
    st.markdown("### 🍿 Recommendation Results (Blind A/B Testing)")
